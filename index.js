...
async function parseCommand(event) {
  if(event.message.text.includes(dateCommand) || event.message.text.includes(dateCommand.toProperCase())) {
    ...
    const keywords=["januari",'februari','maret','april','mei','juni','juli','agustus','september','oktober','movember','desember'];
    let setValues= new Set();
    keywords.forEach(item =>{
      setValues.add(item);
    });
    let regex =/\d+/g;
    if (monthKeyword.length===3 && setValues.has(monthKeyword[1].toLowerCase()) && regex.test(monthKeyword[2])){
    ...
    return createTextResponse("Cara menggunakannya adalah dengan mengetik 'puasa (bulan) (tahun)'.\n\n Contoh input valid : Puasa Desember 2021");
  }
  else {
  return createTextResponse("Keyword Tidak Valid. Ketik 'help' untuk menunjukkan cara penggunaan");}
}
  ...
  let containerJSON = {
    ...
    "body": {
      ...
      "contents": [
        {
          "type": "text",
          "text": `Bulan ${monthKeyword[1].toProperCase()+" "+monthKeyword[2]}`,
          "weight": "bold",
          "size": "xl"
        },
        {
